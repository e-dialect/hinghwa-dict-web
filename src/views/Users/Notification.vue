      ...
      <a-tab-pane tab="收件箱" >
        ...
        >
          <div slot="header">
            <a-row  :gutter="[64]">
              <a-col :span="4">
                <a-button type="primary" @click="selected=[...notification.sent.map((item)=>{return item.id})]" style="width: 96px">全选</a-button>
              </a-col>
              <a-col :span="4">
                <a-button type="primary" @click="selected=[]" style="width: 96px">全不选</a-button>
              </a-col>
              <a-col :span="6">
          ...
          <a-divider style="padding-top: 16px" slot="footer">我也是有底线嗒~</a-divider>
        </a-list>
        </a-row>
      </a-tab-pane>
    </a-tabs>
...
export default {
  ...
  methods: {
    ...
    change (id) {
      ...
      a.forEach((i) => {
        if (i === id) flag = true
        else this.selected = [i, ...this.selected]
      })
      if (flag === false) {
